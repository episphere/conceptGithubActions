...
function parseMasterModule() {
    let masterJSON = {};
    let fileList = [];
    files = fs.readdirSync('./jsonsCopyTEST/');
    files.forEach(function (file) {
        if (file.match(/[0-9]{9}.json/) != null) {
            let currJSON = fs.readFileSync('./jsonsCopyTEST/' + file, { encoding: 'utf8' });
    ...
    // let filename = './transformationFiles/Quest-' + timestamp + '_Transformation.json'; // (TEMPORARILY COMMENT OUT)
    let filename = './transformationFilesCopyTEST/Quest-' + timestamp + '_Transformation.json';
    // write file
