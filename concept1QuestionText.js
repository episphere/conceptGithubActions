    ...
    for(let i = 0; i < cluster.length; i++){
        ...
        if(Object.keys(currCollection).length != 0){
            ...
            for(let a = 0; a < objKeys.length; a++){
                ...
                currCollection[key]={'conceptId': cid, 'Question Text': currCollection[key] };
    ...
    if(collections.length == 0  && leaves.length > 0){
        ...
        for(let i = 0; i < collections.length; i++){
            ...
            for(let j = 0; j < collectionsKeys.length; j++){
                ...
                currCollection[collectionsKeys[j]] = collections[i][collectionsKeys[j]]['Question Text'];
                let found = false;
                for(let i = 0; i < jsonList.length; i++){
                    if(jsonList[i].hasOwnProperty('conceptId') && jsonList[i]['conceptId'] == currCollection.conceptId){
                        found = true;
...
async function readFile(fileName){
    ...
    let filenameOutside = 'Quest-' + timestamp + '_Concept_Id_Dict.csv';
